...
- [ ] **Step 3: Wire into render_table** — `$heat = $this->heatmap_stats( $rows, $config, $columns );` must be assigned BEFORE the `if(!empty($config['showHeader']))` block (the header `cell_style` call also passes `$heat`); update every `cell_style` call to `$this->cell_style( $meta, $config, $ri, $ci, $row[ $ci ] ?? '', $heat )` (header call keeps `$ri = -1` and passes `''` value + `$heat`).
...
Step 5 also makes the cell input's `onFocus` guard range-aware so the focus-follow after a shift+arrow move does not collapse the selection back to the anchor cell. The guard reuses the existing `inSelection(ri,ci)` helper (which already falls back on `r2`/`c2` via `??`) and only re-selects when the focused cell is OUTSIDE the current selection rectangle:

```js
onFocus:e=>{if(!selected||!inSelection(ri,ci))select(e,ri,ci);},
```

`keyNav`'s Enter branch and arrow branch stay as above — they already pass `shift:!!e.shiftKey`, so a Shift+Arrow keeps extending the range.
