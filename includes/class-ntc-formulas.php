...
final class NTC_Formulas {
	public static function apply( array $rows, array $columns, array $cell_meta, int $average_decimals = 2, string $average_round = 'round' ): array {
		foreach ( $cell_meta as $key => $meta ) {
			if ( empty( $meta['formula'] ) ) { continue; }
			...
			$value = null;
			switch ( $formula ) {
				case 'sum': $value=array_sum($values); break;
				case 'sub':
				case 'subtract': $value=array_shift($values); foreach($values as $v){$value-=$v;} break;
				case 'min': $value=min($values); break;
				case 'max': $value=max($values); break;
				case 'average':
				case 'avg':
			...
			if ( null !== $value ) { $rows[$r][$c]=(string)$value; }
		}
		return $rows;
	}
	...
	private static function resolve_values( $source, array $rows, int $row_index ): array {
		$values=array();
		if ( is_array($source) ) { $tokens=$source; }
		else { $tokens=preg_split('/\s*,\s*/',(string)$source,-1,PREG_SPLIT_NO_EMPTY); }
		foreach($tokens as $token){
			if ( is_numeric($token) ) {
				$col=max(0,(int)$token-1);
				if(isset($rows[$row_index][$col])){$values[]=(float)self::numeric($rows[$row_index][$col]);}
			} elseif ( preg_match('/^([A-Z]+)(\d+)$/i',(string)$token,$m) ) {
				$col=self::column_index($m[1]); $row=max(0,(int)$m[2]-1);
				if(isset($rows[$row][$col])){$values[]=(float)self::numeric($rows[$row][$col]);}
			}
		}
