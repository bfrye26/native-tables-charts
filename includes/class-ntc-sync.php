...
final class NTC_Sync {
	...
	public static function fetch( string $url ): string {
		...
		if ( strlen( $body ) > self::MAX_BYTES ) {
			throw new Exception( 'Source file exceeds the 2 MB limit.' ); }
		return $body;
	}
	...
	public static function parse( NTC_Repository $repo, string $raw, string $format ): array {
		if ( 'json' === $format ) {
			...
			return array(
				'columns' => $cols,
				'rows'    => array_map( fn( $r ) => array_map( fn( $k ) => ( array_key_exists( $k, (array) $r ) ? $r[ $k ] : '' ), $keys ), $rows ),
			);
		}
		...
		$fh        = fopen( 'php://temp', 'r+' );
		if ( false === $fh ) {
			throw new Exception( 'Could not parse the import.' ); }
		fwrite( $fh, $raw );
		rewind( $fh );
		while ( true ) {
			$record = fgetcsv( $fh, 0, $delimiter, '"', '' );
			if ( false === $record ) {
				break; }
			if ( 1 === count( $record ) && '' === trim( (string) $record[0] ) ) {
				continue; }
			$matrix[] = $record;
		...
		foreach ( $header as $i => $h ) {
			$cols[] = array(
				'id'    => sanitize_key( ! empty( $h ) ? $h : 'c' . ( $i + 1 ) ),
				'label' => sanitize_text_field( ! empty( $h ) ? $h : 'Column ' . ( $i + 1 ) ),
				'type'  => 'auto',
				'unit'  => '',
			); }
		$width = count( $cols );
