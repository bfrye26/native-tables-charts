...
final class NTC_Repository {
	...
	public function create_dataset( string $name, array $columns, array $rows = array(), string $description = '' ): int {
		...
		$wpdb->insert(
			...
			array(
				'name' => sanitize_text_field( $name ?: __( 'Untitled Dataset', 'native-tables-charts' ) ),
		...
		if ( $id && $rows ) { $this->replace_rows( $id, $rows ); }
		return $id;
	}
	...
	public function update_dataset( int $id, array $data ): bool {
		...
		$formats = array( '%s' );
		if ( array_key_exists( 'name', $data ) ) { $update['name'] = sanitize_text_field( (string) $data['name'] ); $formats[] = '%s'; }
		if ( array_key_exists( 'description', $data ) ) { $update['description'] = sanitize_textarea_field( (string) $data['description'] ); $formats[] = '%s'; }
		if ( array_key_exists( 'columns', $data ) ) { $update['columns_json'] = wp_json_encode( $this->sanitize_columns( (array) $data['columns'] ) ); $formats[] = '%s'; }
	...
	public function get_rows( int $dataset_id, int $limit = 0, int $offset = 0 ): array {
		...
		$items = $wpdb->get_results( $sql, ARRAY_A ) ?: array(); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		$out = array();
		foreach ( $items as $item ) { $out[] = $this->decode( $item['row_json'], array() ); }
		return $out;
	}
	...
	public function upsert_rows( int $dataset_id, array $rows, int $start_index = 0 ): bool {
		...
		foreach ( $rows as $i => $row ) {
			...
			if ( false === $wpdb->query( $sql ) ) { return false; } // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		}
		$this->touch_dataset( $dataset_id );
		return true;
	}
	...
	public function list_views( ?int $dataset_id = null, string $type = '' ): array {
		...
		if ( null !== $dataset_id ) { $where[]='dataset_id=%d'; $args[]=$dataset_id; }
		...
		foreach ( $rows as &$row ) { $row['config'] = $this->decode( $row['config_json'], array() ); unset( $row['config_json'] ); }
		return $rows;
	}
	...
	public function create_view( int $dataset_id, string $type, string $name, array $config ): int {
		global $wpdb;
		$now = current_time( 'mysql', true );
		$wpdb->insert( $wpdb->prefix . 'ntc_views', array(
			'dataset_id'=>$dataset_id, 'name'=>sanitize_text_field( $name ?: ucfirst( $type ) ),
			'type'=> in_array( $type, array( 'table','chart' ), true ) ? $type : 'table',
			'config_json'=>wp_json_encode( $this->sanitize_config( $config ) ), 'author_id'=>get_current_user_id(),
	...
	public function update_view( int $id, array $data ): bool {
		global $wpdb;
		$update = array( 'updated_at'=>current_time( 'mysql', true ) ); $formats=array('%s');
		if ( isset( $data['name'] ) ) { $update['name']=sanitize_text_field( $data['name'] ); $formats[]='%s'; }
		if ( isset( $data['config'] ) ) { $update['config_json']=wp_json_encode( $this->sanitize_config( (array)$data['config'] ) ); $formats[]='%s'; }
		if ( isset( $data['dataset_id'] ) ) { $update['dataset_id']=absint($data['dataset_id']); $formats[]='%d'; }
	...
	public function get_preset( int $id ): ?array {
		global $wpdb;$row=$wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}ntc_presets WHERE id=%d",$id),ARRAY_A);if(!$row)return null;$row['settings']=$this->decode($row['settings_json'],array());unset($row['settings_json']);return $row;
	}

	public function list_presets( string $type = '' ): array {
		global $wpdb;
		$sql = "SELECT * FROM {$wpdb->prefix}ntc_presets";
		if ( '' !== $type ) { $sql .= $wpdb->prepare( ' WHERE type=%s', $type ); }
		$sql .= ' ORDER BY name ASC';
		$rows = $wpdb->get_results( $sql, ARRAY_A ) ?: array(); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
		foreach ( $rows as &$row ) { $row['settings']=$this->decode($row['settings_json'],array()); unset($row['settings_json']); }
		return $rows;
	}
	...
	public function delete_preset( int $id ): bool {
		global $wpdb; return false !== $wpdb->delete($wpdb->prefix.'ntc_presets',array('id'=>$id),array('%d'));
	}

	private function touch_dataset( int $dataset_id ): void {
		global $wpdb; $wpdb->update($wpdb->prefix.'ntc_datasets',array('updated_at'=>current_time('mysql',true)),array('id'=>$dataset_id),array('%s'),array('%d'));
	}

	public function sanitize_columns( array $columns ): array {
		$out=array();
		foreach ( array_slice( array_values($columns), 0, 40 ) as $i=>$col ) {
			if ( ! is_array($col) ) { $col=array('label'=>(string)$col); }
			$out[] = array(
	...
	public function sanitize_row( array $row ): array {
		...
		foreach ( array_slice(array_values($row),0,40) as $cell ) {
			if ( is_scalar($cell) || null === $cell ) { $out[] = wp_check_invalid_utf8( str_replace( "\0", '', (string) $cell ), true ); }
			else { $out[] = wp_json_encode($cell); }
		}
		return $out;
	}
	...
	public function sanitize_config( array $config ): array {
		...
		$clean = function( $value ) use ( &$clean ) {
			if ( is_array($value) ) { $out=array(); foreach($value as $k=>$v){ $safe_key=preg_replace('/[^A-Za-z0-9_:\-]/','',(string)$k); $out[$safe_key]=$clean($v); } return $out; }
	...
	private function decode( string $json, array $fallback ): array {
		$data=json_decode($json,true); return is_array($data)?$data:$fallback;
	}
}
